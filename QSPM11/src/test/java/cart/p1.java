...
public class p1
{
	...
	WebDriver driver; UtilityMethods um=new UtilityMethods();

	@Test public void report() { ExtentReports reports=new ExtentReports();
	ExtentSparkReporter extentsparkreporter=new
			ExtentSparkReporter("./ExtentReports/ "+UtilityMethods.getCurrentTime() +".html"); 
	reports.attachReporter(extentsparkreporter); 

	ExtentTest extentTest= reports.createTest("login"); WebDriver driver=new ChromeDriver();
	extentTest.log(Status.INFO,"browser is opened");
	driver.manage().window().maximize();
	extentTest.log(Status.INFO,"browser is maximized");
	driver.get("https://demowebshop.tricentis.com/");
	driver.findElement(By.linkText("Log in")).click(); WebElement
	emailmailtextbox = driver.findElement(By.id("Email"));
	...
	try {
		...
		extentTest.pass(MediaEntityBuilder.createScreenCaptureFromPath(um.
				takescreenshot(driver)).build()); 
		}
		}
	catch (Exception e) 
	{
					extentTest.fail(MediaEntityBuilder.createScreenCaptureFromPath(um.
					takescreenshot(driver)).build());
